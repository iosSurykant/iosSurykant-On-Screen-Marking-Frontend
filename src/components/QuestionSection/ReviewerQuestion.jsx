...
const ReviewerQuestion = (props) => {
  ...
  const handleNextBooklet = async () => {
    try {
      ...
      const nextBooklet = res?.data?.find((t) => t._id === taskId)
        ?.answerPdfDetails?.[updatedTask.currentFileIndex];
  ...
  const rollback = async () => {
    ...
    if (remarkType === "annotation") {
      ...
      remarkPayload = {
        ...
        annotationType: annotationType, // ✅ NEW FIELD
      };
    }
    ...
    try {
        ...
        assignments: [
          {
            ...
            bookletsToAssign: [currentBookletId],
            remark: remarkPayload, // ✅ NEW FIELD
          },
        ],
      };
      ...
      // const res = handleIndexChange(taskId, nextIndex);
      // dispatch(setCurrentBookletIndex(nextIndex));
  ...
  return (
    <div className="h-[100%] ">
      ...
      <div className="relative h-[90%] overflow-hidden shadow-md sm:rounded-lg">
        {/* Scrollable content */}
